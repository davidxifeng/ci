...
impl Parser {
	...
	fn array_dimensions(&mut self, base_type: Type) -> Result<Type, ParseError> {
		let expr = self.parse_expr(Precedence::P2Assign)?;
		self.expect_punct(Punct::BrakR)?;
		let ty = self.type_suffix(base_type)?;
		Ok(ty.to_array(expr))
	}
	...
	fn declarator(&mut self, base_type: Type) -> Result<Object, ParseError> {
		let mut obj_type = base_type;
		while let Some(Token::Punct(Punct::Mul)) = self.peek() {
			self.advance();
			obj_type = obj_type.to_pointer();
		}
		...
		if let Some(token) = self.peek() {
			...
			Ok(Object::Variable(Variable { next: None, name, ctype: obj_type }))
		} else {
	...
	pub fn test_global_variable(&mut self) -> Result<Option<Object>, ParseError> {
		...
		let obj = self.declarator(base_type)?;
		Ok(Some(obj))
	}
