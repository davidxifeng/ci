...
impl TokenApi {
	...
	fn try_next_token(iter: &mut std::str::Chars, ts: &mut TokenState) -> Option<Result<Token, LexError>> {
	...
	pub fn parse(input: &str) -> Result<Vec<Token>, LexError> {
		...
		let mut lex_state = TokenState { line: 1, token_count: 0 };
		let mut iter = input.chars();
		while let Some(result) = Self::try_next_token(&mut iter, &mut lex_state) {
			match result {
